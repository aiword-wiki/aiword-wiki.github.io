...
refs:[],videos:[],updated:'2026-04-04'},
  ...
  {id:'long-context',t:'롱 컨텍스트',en:'Long Context',c:'model',h:3,born:'2023-11',tags:['컨텍스트윈도우','긴문서','메모리'],
sum:'수십만~수백만 토큰을 한 번에 처리할 수 있는 LLM의 능력. 긴 코드베이스, 책 한 권, 긴 대화를 통째로 모델에 넣어 처리할 수 있게 해준다.',
det:`<h4>개념 설명</h4><p>초기 GPT-3는 약 4,000 토큰 정도만 처리할 수 있어 긴 문서를 잘라야 했다. 이제 Claude는 200K, Gemini는 1M 토큰 이상을 처리한다. <strong>롱 컨텍스트</strong>가 가능해지면서 수만 줄짜리 코드베이스를 한 번에 분석하거나, 긴 계약서 전체를 검토하거나, 수백 번의 대화 히스토리를 유지하는 것이 현실이 됐다.</p><h4>사용 예시</h4><p>Claude Code에서 대형 모노레포를 분석할 때 여러 파일을 컨텍스트에 한꺼번에 넣어 파일 간 의존성을 추적한다. 긴 회의록이나 문서 전체를 넣고 \"3페이지에서 언급된 결정 사항만 뽑아줘\"처럼 정확한 참조도 가능해졌다.</p>`,
rel:['context-window','kv-cache','flash-attention','rag','prompt-caching'],
refs:[],videos:[],updated:'2026-04-04'},

  {id:'speculative-decoding',t:'추측적 디코딩',en:'Speculative Decoding',c:'model',h:3,born:'2023-05',tags:['추론속도','최적화','드래프트모델'],
sum:'작은 드래프트 모델이 여러 토큰을 미리 예측하고, 큰 모델이 한 번에 검증해 처리 속도를 높이는 기법. 품질은 유지하면서 지연 시간을 크게 줄인다.',
det:`<h4>개념 설명</h4><p>LLM은 토큰을 한 번에 하나씩 생성하기 때문에 느리다. <strong>추측적 디코딩</strong>은 작고 빠른 모델(드래프트 모델)이 먼저 여러 토큰을 예측한 뒤, 큰 모델이 이를 한 번의 forward pass로 전부 검증한다. 맞으면 그대로 채택하고, 틀린 부분부터 다시 생성한다. 품질 저하 없이 2~3배 빠른 추론이 가능하다.</p><h4>사용 예시</h4><p>vLLM 같은 추론 서버에서 speculative decoding을 활성화하면 같은 GPU로 더 높은 처리량을 낼 수 있다. 코드 자동완성처럼 반복적인 패턴이 많은 태스크에서 특히 효과적이다.</p>`,
...
rel:['rag','ai-agent','tool-use','embedding','vector-db','react-pattern'],
refs:[],videos:[],updated:'2026-04-04'},
...
rel:['ai-agent','planning','orchestration','react-pattern','chain-of-thought','multi-agent'],
refs:[],videos:[],updated:'2026-04-04'},
...
rel:['ai-agent','human-in-the-loop','planning','tool-use','agent-loop','react-pattern'],
refs:[],videos:[],updated:'2026-04-04'},
...
rel:['ai-agent','autonomous-agent','guardrail','agent-eval','sandboxing'],
refs:[],videos:[],updated:'2026-04-04'},
...
rel:['tool-use','mcp','ai-agent','react-pattern','function-calling','api-gateway'],
refs:[],videos:[],updated:'2026-04-04'},
...
rel:['ai-agent','multi-agent','react-pattern','agent-loop','planning'],
refs:[],videos:[],updated:'2026-04-04'},
...
rel:['task-decomposition','react-pattern','tree-of-thought','ai-agent','reasoning-model'],
refs:[],videos:[],updated:'2026-04-04'},
